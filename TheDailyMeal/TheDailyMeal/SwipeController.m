...
@interface SwipeController () <UIScrollViewDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
	...
	if(self.totalPages !=0)	{
        //to load page views
		self.scrollView.contentOffset = CGPointMake(self.currentPageIndex * ([self pageSize].width + 2*self.imageHorizontalSpacing), 0);
	}
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
	...
	@try {
        ...
        if(scrollView == self.scrollView)   {
            ...
            if (m_bRotationInProgress || avoidScroll)
...
- (void)viewDidUnload   {
    [self setSubNavigationTitle:nil];
    [super viewDidUnload];
