...
//    NSString *code = [self getStringFromUrl:q needle:@"code="];
  ...
  if ((token == (NSString *) [NSNull null]) || (token.length == 0)) {
    ...
    if ([_loginDelegate respondsToSelector:@selector(fbDialogLogin:expirationDate:andRedirectUrl:)]) {
      [_loginDelegate fbDialogLogin:token expirationDate:expirationDate andRedirectUrl:redirectUrl];
    }
