...
- (void)viewDidLoad
{
    ...
    afeDateFromLbl.text  = [Utility getStartDate];
...
//    NSString *startDate = [NSString stringWithFormat:@"01/01/%@",[Utility getYearFromDate:now]];
    ...
    if([[NSUserDefaults standardUserDefaults] objectForKey:@"startDateWellSearchPopOver"]){        
        ...
        defaultStartdate = [dateFormatter dateFromString:afeDateFromLbl.text];
...
-(IBAction)searchBtnTouched{
    ...
    NSLog(@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"startDateWellSearchPopOver"]);
...
- (IBAction)didPickerValueChanged:(id)sender 
{
    switch (datePickr.tag) {
        ...
        case Begin_Date_Picker_Tag:{
            ...
            NSString *beginDate = [self DateFormater:[datePickr date]];
            
            self.afeDateFromLbl.text = beginDate;
...
-(IBAction)textFieldValueChanged:(UITextField *)sender{
        //NSLog(@"%@",wellCompnNamTextView.text);
    if(isSearchEnd == NO){
        //scrollViewContainer.scrollEnabled= YES;
...
{
    NSLog(@"%@",self.apiRequestInfoObjectArray);
    NSLog(@"%d",[apiRequestInfoObjectArray count]);
    if(apiRequestInfoObjectArray){
