...
{
    if(!afeTableView)
        afeTableView = [[BudgetedAFETableView alloc]initWithFrame:CGRectMake(425, 78, 526, 600)];
    else
        [afeTableView removeFromSuperview];
    
    afeTableView.delegate = self;
    [self addSubview:afeTableView];
    [afeTableView refreshTableWithAFEArray:afeArray_Table forPage:currentPageShown ofTotalPages:totalNoPagesAvailable];
}
...
-(void)drawBarChartWithAFEArray:(NSMutableArray *)modelArray{
    ...
    {  
        ...
        CGRect subFrame = CGRectMake(0, 36+frameOffSet, 0, 15);
        ...
        if(!accrualsbarImageView)
            accrualsbarImageView = [[UIImageView alloc]initWithFrame:CGRectMake(actualbarImageView.frame.origin.x+actualbarImageView.frame.size.width+0, 36+frameOffSet, 0, 15)];
        else
                     ...
                     for (UIImageView *barImgeView in barImageArray) 
                         ...
                         if(barImgeView.tag >=100)
                             ...
                             if(barImgeView.tag %3 ==2)
                                 {
                                 CGRect tempFrame = CGRectMake(barImgeView.frame.origin.x+barImgeView.frame.size.width+10, barImgeView.frame.origin.y-8, 250, 30);
...
-(float) getScalingFactor
{
    ...
    if(budgetActualArray && [budgetActualArray count])
        ...
        if([[budgetActualArray objectAtIndex:0] doubleValue] > 170)
            {
            NSLog(@"%f",  [[budgetActualArray objectAtIndex:0] doubleValue]);
