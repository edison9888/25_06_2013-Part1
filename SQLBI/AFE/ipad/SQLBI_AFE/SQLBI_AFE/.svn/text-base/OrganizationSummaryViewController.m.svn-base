...
-(void) initalizeSummaryViewControllers;
...
{
    NSString *resultString = @"Data Warehouse - AFE Dashboard";
...
//    [self initializeOrgSummaryAPIHandlerAndRequestArray];
//    
//    [self stopAPICallOfType:RVAPIRequestTypeGetAFEClass withTag:nil];
//    
//    if(self.apiHandlerOrgSummary)
//    {
//         RVAPIRequestInfo *tempRequestInfo = [self.apiHandlerOrgSummary getAFEClassesOfOrganisation:self.currentlySelectedOrgID withStatus:self.currentlySelectedStatus fromStartDate:[Utility formatDateforAFEAPICalls:self.currentlySelectedStartDate] toEndDate:[Utility formatDateforAFEAPICalls:self.currentlySelectedEndDate] sortedBy:@"Budget" withSortDirection:AFESortDirectionDescending atPageNumber:1 recordLimitPerPage:50];
//      
//        if(!self.apiRequestInfoObjectArray)
//            self.apiRequestInfoObjectArray = [[NSMutableArray alloc] init];
//        
...
//    if(self.apiHandlerOrgSummary)
//    {
//        RVAPIRequestInfo *tempRequestInfo = [self.apiHandlerOrgSummary getAFEsOfOrganisation:self.currentlySelectedOrgID withStatus:self.currentlySelectedStatus fromStartDate:[df stringFromDate:self.currentlySelectedStartDate] toEndDate:[df stringFromDate:self.currentlySelectedEndDate] sortedBy:SORTFIELD_AFEEstimate withSortDirection:AFESortDirectionDescending atPageNumber:1 recordLimitPerPage:50];
//        
//        if(!self.apiRequestInfoObjectArray)
//            self.apiRequestInfoObjectArray = [[NSMutableArray alloc] init];
//        
//        tempRequestInfo.tag = TAG_REQUESTINFO_TOPBUDGETED;
//        [self.apiRequestInfoObjectArray addObject:tempRequestInfo];
//    }
...
-(void) getAFEArrayForProjectWatchlistFromService
{
    ...
    if(self.apiHandlerOrgSummary)
    {
        RVAPIRequestInfo *tempRequestInfo = [self.apiHandlerOrgSummary getAFEsOfOrganisation:self.currentlySelectedOrgID withStatus:self.currentlySelectedStatus fromStartDate:[df stringFromDate:self.currentlySelectedStartDate] toEndDate:[df stringFromDate:self.currentlySelectedEndDate] sortedBy:SORTFIELD_PercentageConsumption withSortDirection:AFESortDirectionDescending atPageNumber:1 recordLimitPerPage:50];
        
        if(!self.apiRequestInfoObjectArray)
            self.apiRequestInfoObjectArray = [[NSMutableArray alloc] init];
        
        tempRequestInfo.tag = TAG_REQUESTINFO_PROJECTWATHCLIST;
        [self.apiRequestInfoObjectArray addObject:tempRequestInfo];
    }
     */
...
-(void) didReceiveAFEClassesArrayForTopAFEClassesFromService
{
    if(topAFEClassSummaryView)
    {
        ...
        NSLog(@"%@",afeClassesArrayForTopAFEClasses);
        [topAFEClassSummaryView refreshPieChartWithAFEClassesArray:afeClassesArrayForTopAFEClasses andStartDate:self.currentlySelectedStartDate andEndDate:self.currentlySelectedEndDate];
    }
    ...
    switch (requestInfoObj.requestType) {
        ...
        case RVAPIRequestTypeGetAFEsByClass:
        {
            ...
            if([((NSString*)requestInfoObj.tag) caseInsensitiveCompare:TAG_REQUESTINFO_AFEForBarchartInTopBudgetedAFESummaryView] == NSOrderedSame)
            {
                ...
                //NSLog(@"AFEARRAY: %@", afeArrayForTopBudgetedAFE);
            ...
            else if([((NSString*)requestInfoObj.tag) caseInsensitiveCompare:TAG_REQUESTINFO_AFEForHeatMapInProjectWatchlistSummaryView] == NSOrderedSame)
            {
                ...
                //NSLog(@"AFEARRAY: %@", afeArrayForProjectWatchlist);
...
-(void) showDetailedViewControllerForPageType:(NSNumber*) pageType
{
    AFEOrganizationSummaryPageType actualPageType = [pageType intValue];
    
    if(!detailsVC)
    {
        detailsVC = [[AFEOrganizationDetailsViewController alloc] initWithOrganizationSummaryPageSeleted:actualPageType];
    }
...
-(void) didClickShowSummaryButtonOnDetailsViewController:(AFEOrganizationDetailsViewController*) controller
{
    ...
    {
        [self removeDetaildViewWithAnimation:YES];
