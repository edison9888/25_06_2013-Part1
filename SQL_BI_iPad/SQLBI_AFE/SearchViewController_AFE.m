...
{
    [self popOverViewTouched:nil];
    if([afenameField.text isEqualToString:@""] && [afenumberField.text isEqualToString:@""]){
        ...
        if(self.selectedID == NULL || [self.selectedID isEqualToString:@""]){
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"Fields Empty" message:@"Please Enter a Valid AFE Number or AFE Name" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
            [alertView show];
            ...
            if(self.delegate && [self.delegate respondsToSelector:@selector(searchWithDataAFEID:)]){
                for(AllAFEs *tempAllAFE in resultArray){
                    if([tempAllAFE.afeID caseInsensitiveCompare:self.selectedID] == 0){
                        [[NSUserDefaults standardUserDefaults] setObject:tempAllAFE.afeNumber forKey:@"afeNumber"];
                        [[NSUserDefaults standardUserDefaults] setObject:tempAllAFE.afeName forKey:@"afeName"];
                        [[NSUserDefaults standardUserDefaults] setObject:tempAllAFE.afeID forKey:@"afeID"];
                        break;
                    }
                }
