...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    if(!isUpgradeUIOnScreen)
    {
        [AppUpdateManager sharedManager].delegate = self;
        [[AppUpdateManager sharedManager] performSelector:@selector(checkForNewVersion) withObject:nil afterDelay:3];
    }
...
-(void) jumpToAFESearchAndSearchAFEWithObject:(AFE*) afeObject
{
    if(self.tabBarController && afeObject)
    {
        ...
        if(afeSearchVC)
            [afeSearchVC performSelector:@selector(searchAFEDetailsWithAFEObject:) withObject:afeObject afterDelay:0.1];
        
    }
}


#pragma mark - AppUpdateManager Delegate methods
...
-(void) didFindNewVersionOfTheApplication:(AppVersion *)newVersion
{
    ...
    if(!isUpgradeUIOnScreen)
    {
        isUpgradeUIOnScreen = YES;
        
        [RVAlertMessageHandler showMultipleButtonAlertWithTitle:@"AFE Analytics" message:@"A new version of the application is available. Do you want to upgrade?" delegateObject:self viewTag:123321 otherButtonTitle:[NSArray arrayWithObjects:@"Cancel", @"Upgrade", nil]];
...
{
    //[RVAlertMessageHandler showDiscardAlertWithTitle:@"AFE Analytics" message:@"Wrong Application key is used! Kindly contact the Administrator." delegateObject:nil viewTag:1];
}

-(void) willInstallTheNewApplication
{
    NSLog(@"Will install new version");
}

-(void) didFailInstallationOfTheNewVersion
{
    NSLog(@"Did fail installation of new version");
}

- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    if(alertView.tag == 123321)
    {
        if(buttonIndex == 0)
            NSLog(@"Canceled Installation.");
        else if(buttonIndex == 1)
        {
            NSLog(@"Chose to Install new version:");
            [[AppUpdateManager sharedManager] installTheLatestVersion];
        }
