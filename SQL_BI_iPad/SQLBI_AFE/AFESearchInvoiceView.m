...
-(void) awakeFromNib{
    ...
    sortingParameterTypeArrays = [[NSArray  alloc]initWithObjects:SORTFIELD_InvoiceNumber,SORTFIELD_BillingCategoryName,SORTFIELD_InvoiceDate,SORTFIELD_AFEInvoiceAmount,SORTFIELD_PropertyName,SORTFIELD_PropertyType,SORTFIELD_ServiceDate,SORTFIELD_AccountingDate,SORTFIELD_VendorName, nil];
...
-(void)getAfeSearchInvoiceArray:(NSArray *)afeInvoiceArray forPage:(int) page ofTotalPages:(int) totalPages{
    ...
    if(curntRecordNmbr >totalRecords)
        curntRecordNmbr = [afeInvoiceArray count];
