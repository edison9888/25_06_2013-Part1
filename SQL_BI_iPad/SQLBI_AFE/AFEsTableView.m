...
-(void) refreshTableWithAFEArray:(NSArray*) afeArrayToUse forPage:(int) page ofTotalPages:(int) totalPages  andTotalRecords:(int) totalRecords
{
   ...
   currentPageShown = page;
    ...
    if(page <= currentPageShown)
        currentRecordCount = currentRecordCount  + self.afeArray.count;
    if(currentPageShown <= 1)
       currentRecordCount = 0;
    if(self.afeArray && self.afeArray.count > 0){
        self.displayLbl.text = [NSString stringWithFormat:@"Displaying %d - %d of %d",K_LIST_COUNT *(page-1) + 1, K_LIST_COUNT *(page-1)+[self.afeArray count],totalRecords];
    }
    else
    ...
    sortingParameterTypeArray = [[NSArray  alloc]initWithObjects:SORTFIELD_AFEName,SORTFIELD_Class,SORTFIELD_AFEEstimate,SORTFIELD_Actual,SORTFIELD_Accrual,SORTFIELD_Total, nil];
    parameterTypeArray = [[NSArray alloc]initWithObjects:@"castString",@"castString",@"castInt",@"castInt",@"castInt",@"castInt", nil];
