...
- (void)connectionDidFinishLoading:(NSURLConnection*)theConnection {
    ...
    UIImageView* imageView = [[[UIImageView alloc] init] autorelease];
    imageView.frame = CGRectMake(2, 3, self.frame.size.width-4, self.frame.size.height-8); 
    imageView.image = [[UIImage imageWithData:data] resizedImage:CGSizeMake(self.frame.size.width-2, self.frame.size.height-8)];     
    imageView.contentMode = UIViewContentModeScaleAspectFit;
    imageView.autoresizingMask = (UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight);
    ...
    channelDetail.channelIcon = image;  // save the image to avoid multiple downloads
