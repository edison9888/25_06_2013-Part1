...
- (void)updateDownloadProgress
{
	...
	double progressAmount  = (([self totalBytesRead]+[self partialDownloadSize])*1.0)/(([self contentLength]+[self partialDownloadSize])*1.0);
	if (self.delegate!=nil&&[self.delegate respondsToSelector:@selector(downloadProgress:Progress:)]) {
		[self.delegate downloadProgress:self Progress:progressAmount];
	}
