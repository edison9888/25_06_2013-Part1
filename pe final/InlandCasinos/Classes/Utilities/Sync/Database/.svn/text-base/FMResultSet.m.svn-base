    ...
    do {
        ...
        if (SQLITE_BUSY == rc) {
            ...
            if ([parentDB busyRetryTimeout] && (numberOfRetries++ > [parentDB busyRetryTimeout])) {
                ...
                DebugLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [parentDB databasePath]);
            ...
            DebugLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            DebugLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        }
...
- (int) columnIndexForName:(NSString*)columnName {
    ...
    DebugLog(@"Warning: I could not find the column named '%@'.", columnName);
