...
- (id)initWithPhotos:(NSMutableArray *)photosArray {
	...
	if ((self = [super init])) {
        ...
        photos=[[NSMutableArray alloc]init];
         for (NSString *filePath in photosArray) {
                    [photos addObject:[MWPhoto photoWithFilePath:filePath]];
        }
    ...
    //self.navigationItem.backBarButtonItem.title = @"Story";
	return self;
}

-(void)refreshView:(NSNotification *)notification{
   // [photos addObject:[notification object]];
}
...
- (void)viewDidLoad {
    ...
    if (photos.count > 0) {
        ...
        if (photos.count > 0) [items addObject:previousButton];
        [items addObject:space];
        if (photos.count > 0) [items addObject:nextButton];
        [items addObject:space];
...
- (void)tilePages {
	...
	for (ZoomingScrollView *page in visiblePages) {
		if (page.index < (NSUInteger)iFirstIndex || page.index > (NSUInteger)iLastIndex) {
			...
			/*DebugLog(@"Removed page at index %i", page.index);*/
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
			...
			/*DebugLog(@"Added page at index %i", page.index);*/
		}
	}
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) { [(MWPhoto *)[photos objectAtIndex:i] releasePhoto]; /*DebugLog(@"Release image at index %i", i);*/ }
        
        // Preload index - 1
        i = index - 1; 
        if (i < photos.count) { [(MWPhoto *)[photos objectAtIndex:i] obtainImageInBackgroundAndNotify:self]; /*DebugLog(@"Pre-loading image at index %i", i);*/ }
        
    }
    if (index < photos.count - 1) {
        
        // Release anything > index + 1
        for (i = index + 2; i < photos.count; i++) { [(MWPhoto *)[photos objectAtIndex:i] releasePhoto]; /*DebugLog(@"Release image at index %i", i);*/ }
...
- (void)hideControls { [self setControlsHidden:!hideTabBar]; }
- (void)toggleControls { [self setControlsHidden:!hideTabBar]; }
