...
-(NSString *)getRootDirectory{
	...
	filename[fileInfo.size_filename] = '\0';
	NSString * strPath = [NSString  stringWithCString:filename encoding: NSUTF8StringEncoding];
	NSArray *objArr=[strPath componentsSeparatedByString:@"\\"];
	if ([objArr count]>0) {
	return [objArr objectAtIndex:0];
	}
	return nil;*/
	// reading data and write to file
    BOOL ret;
    ...
    filename[fileInfo.size_filename] = '\0';
    NSString *strPath = [NSString  stringWithCString:filename encoding: NSUTF8StringEncoding];
    NSString *strOrginalPath=[strPath stringByReplacingOccurrencesOfString:@"\\" withString:@"/"];
    NSArray *objArr=[strOrginalPath componentsSeparatedByString:@"/"];
    if ([objArr count]>0) {
		return [objArr objectAtIndex:0];
    }
    return nil;
